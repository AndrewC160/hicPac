...
patchwork_ogm_contigs  <- function(tb_ogm_in,regions_tb=NULL,hic_file=NULL,gr_segments=NULL,label_contigs=FALSE,label_xmaps=FALSE,contig_size=3,xmap_size=2,junctions_only=TRUE,return_tables_only=FALSE){
  filter  <- dplyr::filter
  rename  <- dplyr::rename
  mutate  <- dplyr::mutate
  select  <- dplyr::select
